...
class UserTokenService
{
    ...
    {
        $time = time();
        $userSession = ErpUserSession::where([['user_id', '=', $userId], ['terminal', '=', $terminal]])->find();
        ...
        if ($userSession) {
            ...
            $userSession = ErpUserSession::create([
                'user_id' => $userId,
                'terminal' => $terminal,
    ...
    {
        $time = time();
        $adminSession = ErpUserSession::where('token', '=', $token)->find();
        //延长token过期时间
    ...
    {
        $userSession = ErpUserSession::where('token', '=', $token)
            ->find();
