...
use app\common\model\{ErpOrderProduct,ErpOrderAccessory,ErpOrderAftersale,ErpOrderProduceProcess,ErpProcessWage,ErpProduct,ErpProductStock};
...
class ErpOrderLogic extends BaseLogic{
    ...
    public static function getList($query=[],$limit=10){
		$field = 'id,order_sn,sale_order_id,create_time,customer_id,salesman_name,customer_name,region_type,is_special,order_status,contacts,address,address_short,phone,shipping_type,order_amount,order_product_num,shipping_status,salesman_id,produce_status,delivery_time,technician_approve,salesman_approve,data_type';
        $list = ErpOrder::withSearch(['query'],['query'=>$query])->with(['sale_order'])->field($field)->order('id','desc')->append(['shipping_type_desc','order_status_desc','shipping_status_desc','produce_status_desc','region_type_desc','create_date','can_cancel','can_remove','technician_approve_desc','salesman_approve_desc','sale_order_sn'])->paginate($limit);
        return ['code'=>0,'data'=>$list->items(),'extend'=>['count' => $list->total(), 'limit' => $limit]];
    }

	public static function getExportCount($query=[]){
        $count 			= ErpOrder::withSearch(['query'],['query'=>$query])->field('id')->count();
	...
	public static function getExport($query=[],$limit=10000){
		...
		if(!empty($query['data_type']) && $query['data_type'] == 2){
			$field 	= ['region_type_desc'=>'订单性质','customer_name'=>'客户名称/订货单位','order_sn'=>'售后单号','sale_order_sn'=>'关联销售单号','address'=>'收货地址/国家','order_product_num'=>'订购总数量','salesman_name'=>'销售员','create_date'=>'下单日期','delivery_time'=>'交货日期','order_status_desc'=>'订单状态','shipping_status_desc'=>'发货状态'];
		}else{
			$field 	= ['region_type_desc'=>'订单性质','customer_name'=>'客户名称/订货单位','order_sn'=>'销售合同号','address'=>'收货地址/国家','order_product_num'=>'订购总数量','order_amount'=>'合计金额','salesman_name'=>'销售员','create_date'=>'下单日期','delivery_time'=>'交货日期','order_status_desc'=>'订单状态','shipping_status_desc'=>'发货状态','produce_status_desc'=>'生产状态'];
		}
	...
	public static function getOrderStatusCount($data_type=ErpOrderEnum::DATA_TYPE_1){
		$status 		= ErpOrderEnum::getOrderStatusDesc();
		$data 			= [];
		foreach($status as $key=>$vo){
			$data[$key]	= ['name'=>$vo,'value'=>'?order_status='.$key,'count'=>ErpOrder::where('order_status',$key)->where('data_type','=',$data_type)->count()];
		}
		if($data_type == ErpOrderEnum::DATA_TYPE_1){
			$data[11]	= ['name'=>'待技术审批','value'=>'?technician_approve=0','count'=>ErpOrder::where('technician_approve',0)->where('data_type','=',$data_type)->count()];
		}
		$data[12]		= ['name'=>'待销售审批','value'=>'?salesman_approve=0&technician_approve=1','count'=>ErpOrder::where('salesman_approve',0)->where('technician_approve',1)->where('data_type','=',$data_type)->count()];
		ksort($data);
		return $data;
	}

    // 添加
    public static function goAdd($data,$data_type=ErpOrderEnum::DATA_TYPE_1)
    {
        //验证
        ...
        try {
			$count 						= ErpOrder::withTrashed()->whereDay('created_at')->where('data_type','=',$data_type)->count() + 1;
			$data['order_sn']			= ($data_type==ErpOrderEnum::DATA_TYPE_2?'SH':'XS').date('y-m-d').'-'.sprintf("%03d",$count);
			$data['salesman_id'] 		= self::$adminUser['id'];
			$data['salesman_name'] 		= self::$adminUser['username'];
			$data['technician_approve'] = $data['is_special']==0?1:0;
    ...
    public static function goEdit($data)
    {
		...
		if($model['is_special'] != $data['is_special']){
			$data['technician_approve'] = $data['is_special']==1?0:1;
		}

        try {
			$log 			= [];
			$filed_check   	= ['order_remark'=>'订单备注','delivery_remark'=>'交货备注','delivery_time'=>'交货日期','address'=>'收货地址/国家','address_short'=>'收货地址/国家(简称)','contacts'=>'联系人','phone'=>'联系电话','cabinet_num'=>'装柜号','technical_parameter'=>'技术参数','motor_code'=>'电机编码','shipping_type'=>'发货类型','region_type'=>'发货类型','is_special'=>'特殊订单','order_type'=>'类型'];
			foreach($filed_check as $k=>$vo){
				if(isset($data[$k]) && $data[$k] != $model[$k]){
					...
					if($k == 'shipping_type'){
						...
						$before	= ErpOrderEnum::getOrderTypeDesc($model[$k]);
						$after 	= ErpOrderEnum::getOrderTypeDesc($data[$k]);
					}else{
						$before = $model[$k];
						$after 	= $data[$k];
					}
    ...
    public static function goRecycle($ids,$action)
    {
		...
		try{
			if($action){
				...
				ErpOrderAftersale::where('order_id','in',$ids)->delete();
			}
		}catch (\Exception $e){
			return ['msg'=>'操作失败'.$e->getMessage(),'code'=>201];
		}
	...
	public static function  goSalesmanPass($ids){
		if(!is_array($ids)){
			$ids = [$ids];
		}
		foreach($ids as $id){
			self::_goSalesmanPass($id);
		}
	}
	
	
	public static function  _goSalesmanPass($id){
		$model 		= self::getOne($id);
        if(empty($model['id'])) {
			return ['msg'=>'订单不存在','code'=>201];
		}
		...
		if($model->data_type == ErpOrderEnum::DATA_TYPE_1 && $model->order_status != ErpOrderEnum::ORDER_STATUS_WAIT_HANDLE && $model->order_status != ErpOrderEnum::ORDER_STATUS_WAIT_PRODUCE){
			return ['msg'=>'订单已排产','code'=>201];
		}
		Db::startTrans();
		try{
			...
			ErpOrderLog::create(['log'=>'销售审核订单：'.$model['order_sn'],'data_type'=>ErpOrderLogEnum::SALESMAN_PASS,'order_id'=>$model['id'],'operator'=>self::$adminUser['username']]);
			if($model->data_type == ErpOrderEnum::DATA_TYPE_1){
				$date							= date('ym');
				$wage_data 						= [];
				$tmp 							= ErpProcessWage::alias('a')->join('erp_process b','a.process_id = b.id','LEFT')->with(['process'])
				->where('a.product_id','in',$model->order_product->column('product_id'))
				->whereNull('b.delete_time')
				->whereNotNull('b.id')
				->field('a.*,b.name as process_name')
				->order(['b.sort'=>'asc','b.id'=>'asc'])->select();
				$wage 							= [];
				foreach($tmp as $vo){
					$wage[$vo['product_id']][] 	= $vo;
				}
				$product 						= ErpProduct::where('id','in',$model->order_product->column('product_id'))->column('id,udi,version,id_code','id');
				$product_stock_id 				= [];
				foreach($model->order_product as $vo){
					$produce 					= [];
					$count 						= ErpOrderProduce::withTrashed()->whereMonth('create_time')->count();
					for($i=1;$i<=$vo['product_num'];$i++){
						$count					= $count + 1;
						$produce[] 				= ['produce_type'=>$vo['product_stock_id']?ErpOrderProduceEnum::PRODUCE_TYPE_3:ErpOrderProduceEnum::PRODUCE_TYPE_1,'queue_num'=>$i,'udi'=>$product[$vo['product_id']]['udi'],'version'=>$product[$vo['product_id']]['version'],'product_id'=>$vo['product_id'],'order_id'=>$vo['order_id'],'order_product_id'=>$vo['id'],'produce_sn'=>$product[$vo['product_id']]['id_code'].$date.sprintf("%04d",$count)];	
						if($vo['product_stock_id']){
							$product_stock_id[]	= $vo['product_stock_id'];
						}
					}
					$res 						= (new ErpOrderProduce)->saveAll($produce);
					foreach($res as $v1){
						if(!empty($wage[$v1['product_id']])){
							foreach($wage[$v1['product_id']] as $v2){
								$wage_data[] 	= ['price'=>$v2['price'],'process_id'=>$v2['process_id'],'process_name'=>$v2['process_name'],'order_produce_id'=>$v1['id'],'product_id'=>$vo['product_id'],'order_id'=>$vo['order_id']];
							}
						}	
					}
				}
				if($wage_data){
					(new ErpOrderProduceProcess)->saveAll($wage_data);
				}
				if($product_stock_id){
					ErpProductStock::where('id','in',$product_stock_id)->update(['is_out_warehouse'=>1]);
				}
			}
			Db::commit();
        }catch (\Exception $e){
	...
	public static function  goSalesmanReset($id){
		...
		try{
			...
			$log	= ['log'=>'销售反审订单：'.$model['order_sn'],'data_type'=>ErpOrderLogEnum::SALESMAN_RESET,'order_id'=>$model['id'],'operator'=>self::$adminUser['username']];
			ErpOrderLog::create($log);
			...
			if($model->data_type == ErpOrderEnum::DATA_TYPE_1){
				...
				ErpOrderProduceProcess::destroy(function($query) use($model){
					$query->where('order_id',$model['id']);
				});				
	...
	public static function updateAftersale($order_id){
		$update 						= [];
		$update['order_product_num'] 	= ErpOrderAftersale::where('order_id',$order_id)->sum('material_num');
		ErpOrder::where('id',$order_id)->update($update);
	}
